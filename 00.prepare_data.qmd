...
### Format for future prediction

Format the POC concentration data as a dataframe to be used later for predicting POC flux. First, let’s compute an annual climatology.
...
image(poc_ann[,,1], col = cmocean("matter")(100), main = "Surface annual")
```
...
df_poc <- poc_ann %>% as.vector() %>% as.data.frame() %>% setNames("poc")
# Add lon, lat and depth
df_poc$lon <- lon
df_poc$lat <- rep(lat, each=length(lon))
df_poc$depth <- rep(depth, each=length(lon)*length(lat))
# Convert to tibble
df_poc <- df_poc %>% as_tibble() %>% select(lon, lat, depth, poc)
...
Reshape the dataframe to have one column per depth.
...
```{r save}
save(df_all, df_poc, file = "data/00.df_all.Rdata")
```
